...
class TestFDBSingleKeyMutation(unittest.TestCase):
    ...
    def test_mutation_type_enum_values_match_fdb_cdc(self):
        ...
        for name, expected_val in self.EXPECTED_ENUM_VALUES.items():
            with self.subTest(enum_name=name):
                actual_val = MutationType.Value(name)
                self.assertEqual(
                    actual_val,
                    expected_val,
...
class TestStressAndEdgeCases(unittest.TestCase):
    ...
    def test_large_batch_serialization(self):
        ...
        mutations = [
            mutations_pb2.FDBMutation(
                version_index=mutations_pb2.FDBVersionIndex(fdb_version=100_000, sequence_no=i),
                single_key_mutation=mutations_pb2.FDBSingleKeyMutation(
    ...
    def test_max_uint64_version(self):
    ...
    def test_binary_safety_in_keys_and_values(self):
