...
def _check_version(fdb_version: int, index: int | None) -> None:
    # The wire field is uint64; a native commit version is int64_t, and anything
    # larger could not be re-serialized.
...
def _mutation(message: mutations_pb2.FDBMutation, index: int | None) -> Mutation:
    # Presence, not content: an absent version index must never read as (0, 0).
    ...
    if arm == "clear_range":
        # The clear_range arm has no type field; the type code is synthesised.
    ...
    # mutation_type is an open enum: any int32 parses, only a native uint8_t is a
    # type code.
...
def _body(message: mutations_pb2.FDBMutationRecord) -> RecordBody:
    ...
    if arm is None:
        # Also what an arm added by a newer schema looks like: to this reader it
        # is an unknown field.
        raise RecordDecodeError(
...
def _bridge_timestamp_ns(message: mutations_pb2.FDBMutationRecord) -> int | None:
    if not message.HasField("bridge_timestamp"):
        return None
    # Straight from the two fields: the Timestamp helpers range-check, and the
...
def deserialize_record(data: bytes) -> Record:
    ...
    Strict for identity, tolerant for metadata: the record body, version indexes,
    type codes and batch shape are checked, so every returned record body is
    re-serializable. The envelope is returned as found and never fails the call:
    `stream_name` may be `""` and `bridge_timestamp_ns` may be `None` or outside
    the serializer's accepted range.
    ...
    Raises:
        InputTypeError: If `data` is not `bytes`; `field` is `"data"`.
        RecordDecodeError: If the bytes are an undecodable record. `reason` names
            the failed check and `index` the batch position, where there is one.
            For `MALFORMED_WIRE` the protobuf exception is the `__cause__`.
    """
    ...
    try:
        ...
        # upb raises DecodeError for invalid UTF-8; the pure-python backend raises
